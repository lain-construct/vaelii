...
(defn lines-text
  ...
  format, which is the contract `vaelii.host.llm.session/parse-lines` reads.  Each entry
...
(defn json-lines-text
  ...
  `vaelii.host.llm.selection/output-schema` produces — the *other* shape the parser
...
(defn assertions-text
  ...
  `vaelii.host.llm.page/output-schema` constrains decoding to, which is that path's
...
(defn assertion-lines-text
  ...
  shape `vaelii.host.llm.session/parse-assertions` falls back to."
...
(defn candidates-text
  ...
  `vaelii.host.llm.text/output-schema` constrains decoding to.  Each entry is
...
(defn verdicts-text
  ...
  `vaelii.host.llm.oracle/output-schema` constrains decoding to.  Each entry is
