...
(def default-generation-model
  "The model the **page-generation** path runs (`vaelii.host.llm.session/propose-page`)
...
(defn parse-response
  "Ollama's chat JSON -> the neutral response map (`vaelii.host.llm.protocol`)."
  [m]
...
(defn generation-provider
  ...
  (`configured-generation-model`), for `vaelii.host.llm.session/propose-page`.  `opts` is
