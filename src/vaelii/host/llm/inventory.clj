...
(defn specificity
  ...
  Public because `vaelii.host.llm.text` builds a card from a document's resolved terms
...
(defn inventory
  ...
  **`:dropped` counts all three cuts**, because a card that cuts silently is indistinguishable from the
...
(defn- fit
  ...
  worse answer, never a wrong one — so this trims where `vaelii.host.llm.selection` refuses.
...
(defn render
  ...
  The relation block is headed by the instruction that makes it required, since a card
