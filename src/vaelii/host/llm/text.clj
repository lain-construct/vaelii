...
(defn known
  ...
  **One roster read, not a probe apiece.**  `vaelii.host.llm.inventory/unknown-terms` asks
...
(defn declared-in
  ...
  Scoped where `vaelii.host.llm.inventory/declared-arities` is not, and that is the
...
(defn document-inventory
  ...
  `vaelii.host.llm.inventory/inventory`'s shape so its renderer takes it unchanged:
  ...
  **Cost is the vocabulary's, not the document's and not the KB's** — the same shape
  `vaelii.host.llm.inventory/inventory` has, and the same reason: the taxonomy's node set,
...
(defn review-queue
  ...
  (`vaelii.host.llm.inventory/coined`) or a shape `vaelii.host.llm.correct` would rewrite.
