...
(ns vaelii.impl.opts
  "The option-map door: a key nothing reads is refused, and so is an `opts` that is not
  a map.
  ...
  A door with further checks on the *values* of known keys keeps them; this is the key
...
(defn check!
  ...
  passed).  `subject` names the door in the message; `consequence` is the clause on what
  ...
  The second `map?` is redundant to a reader and load-bearing to clj-kondo, which does
