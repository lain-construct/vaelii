...
(defn- determinate-filler?
  "Does the collection `dep` admit at least one determinate filler `y` believed at position
  `arg-pos` of `(pred … x …)` in `ctx`?  `arg-pos` is `:second` for a `predAllSpecified`
  (x at position 1, filler at 2) or `:first` for a `predSpecifiedAll` (x at position 2,
  filler at 1).

  A binding that carries no `?y` is not a filler: the registry answers a goal with `[{}]`
  where a prover proves it without binding anything, and `(dep nil)` is not a membership
  question."
