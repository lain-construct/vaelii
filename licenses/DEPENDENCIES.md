...
chain where a POM inherits one), not from a hand-kept list. It is regenerated by
the maintainer's release tooling, which lives outside this repository; `lein deps
:tree` names the same closure this file describes.
