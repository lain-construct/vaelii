...
(deftest violations-accumulate-across-chaining-runs
  (tu/with-neutral-kb [kb tu/fresh]
    (tu/with-terms [person rock parentOf looksLike unrelated Boulder Fido Spot]
      ...
      ;; the rule derives (parentOf Boulder Fido) for the stored (rock Boulder) — an
      ...
      (v/assert-rule kb [(list rock '?x)] (list parentOf '?x Fido) 'UniverseContext)
...
(deftest a-completed-firing-with-no-placement-context-is-recorded
  (tu/with-neutral-kb [kb tu/fresh]
    (tu/with-terms [dog barksAt Fido IslandAContext IslandBContext]
      ...
      (v/clear-violations! kb)
      (v/assert kb (list dog Fido) IslandBContext)
      (let [drops (filter #(= :no-placement (:violation %)) (v/violations kb))]
        (is (seq drops) "the evaporated firing was recorded")
        (is (= (list barksAt Fido Fido) (:sentence (first drops))))
        (is (empty? (v/sentexes-matching kb (list barksAt Fido Fido) '?ctx))
