  ...
  (tu/with-terms [dog Fido]
    (let [[seen f] (recorder)]
      ...
      (let [e (first (:believed-added (first @seen)))]
        ...
        (is (= (v/handle-of kb (list dog Fido) 'UniverseContext) (:handle e))
...
(tu/deftest-kb a-derived-conclusion-arrives-with-the-rule-that-derived-it
  (tu/with-terms [dog barks Fido]
    (let [[seen f] (recorder)]
      ...
      (v/assert kb (list dog Fido) 'UniverseContext)
      (is (= 1 (count @seen)))
      (is (= #{(list dog Fido) (list barks Fido)} (set (added @seen)))
          "the premise and what followed from it are one event")
      (let [c (first (filter #(= (list barks Fido) (:sentence %))
        ...
        (is (= [(list dog Fido)] (:antecedents (:justification c))))))))
...
(tu/deftest-kb a-batch-settles-once-and-is-one-event
  (tu/with-terms [dog barks Fido Rex Spot]
    (let [[seen f] (recorder)]
      (v/assert kb (vr/rule-sentence [(list dog '?x)] (list barks '?x)) 'UniverseContext)
      (v/watch kb f)
      (v/assert-many kb (mapv #(list dog %) [Fido Rex Spot]) 'UniverseContext)
...
(tu/deftest-kb a-defeat-and-its-revival-both-arrive
  (tu/with-terms [dog barks Fido]
    (let [[seen f] (recorder)]
      ...
      (v/assert kb (list dog Fido) 'UniverseContext)
      (v/watch kb f)
      (let [neg (v/assert kb (list 'not (list barks Fido)) 'UniverseContext
                          {:strength :monotonic})]
        (is (= 1 (count @seen)))
        (is (= [(list barks Fido)] (removed @seen)) "the default lost to known-true content")
        (is (= :defeated (:reason (first (:believed-removed (first @seen))))))
        (reset! seen [])
        (v/retract! kb neg)
        (is (= 1 (count @seen)) "the revival is its own event")
        (is (= [(list barks Fido)] (added @seen)))
        (is (empty? (removed @seen))
...
(tu/deftest-kb a-preview-fires-nothing
  ...
  (tu/with-terms [dog barks Fido]
    (let [[seen f] (recorder)]
      (v/assert kb (vr/rule-sentence [(list dog '?x)] (list barks '?x)) 'UniverseContext)
      (v/watch kb f)
      (let [pv (v/preview kb {:add [[(list dog Fido) 'UniverseContext]]})]
...
(tu/deftest-kb a-rebuild-fires-nothing
  ...
  (tu/with-terms [dog barks Fido]
    ...
    (let [[seen f] (recorder)]
      ...
      (v/recover kb)
      (is (empty? @seen))
      (is (v/ask? kb (list barks Fido) 'UniverseContext) "the rebuild did happen"))))
...
(tu/deftest-kb a-teardown-that-re-derives-what-it-swept-is-still-one-event
  ...
  (tu/with-terms [dog barks sick Fido]
    (v/assert kb (list 'exceptWhen [(list sick '?x)]
                       (list 'set/defaultRule
                             (vr/rule-sentence [(list dog '?x)] (list barks '?x))))
              'UniverseContext)
    (v/assert kb (list dog Fido) 'UniverseContext)
    (let [h (v/assert kb (list sick Fido) 'UniverseContext)]
      (is (not (v/ask? kb (list barks Fido) 'UniverseContext)) "the exception holds")
      (let [[seen f] (recorder)]
        (v/watch kb f)
        (v/retract! kb h)
        (is (= 1 (count @seen)) "one operation, one event")
        (is (= [(list barks Fido)] (added @seen)))
        (is (v/ask? kb (list barks Fido) 'UniverseContext))))))
...
(tu/deftest-kb a-reindex-fires-nothing-either
  ...
  (tu/with-terms [dog Fido]
    ...
    (let [[seen f] (recorder)]
      ...
      (v/reindex kb)
      (is (empty? @seen))
      (is (v/ask? kb (list dog Fido) 'UniverseContext) "the rebuild did happen"))))
...
(tu/deftest-kb an-inert-sentex-arrives-nowhere
  ...
  (tu/with-terms [dog Fido]
    (let [[seen f] (recorder)]
      (v/watch kb f)
      (let [h (v/assert-inert kb (list dog Fido) 'UniverseContext)]
...
(tu/deftest-kb forward-chain-delivers-what-it-derived
  ...
  (tu/with-terms [dog barks Fido]
    (v/assert kb (vr/rule-sentence [(list dog '?x)] (list barks '?x)) 'UniverseContext
              {:chain? false})
    (v/assert kb (list dog Fido) 'UniverseContext {:chain? false})
    (let [[seen f] (recorder)]
      (v/watch kb f)
...
(tu/deftest-kb registering-a-listener-does-not-move-belief
  ...
  (tu/with-terms [dog barks Fido Rex]
    (v/assert kb (vr/rule-sentence [(list dog '?x)] (list barks '?x)) 'UniverseContext)
    (v/assert kb (list dog Fido) 'UniverseContext)
    (let [quiet (mapv #(v/ask? kb % 'UniverseContext)
                      [(list dog Fido) (list barks Fido)])]
      (v/watch kb (fn [_] nil))
...
(tu/deftest-kb a-listener-that-asserts-is-delivered-its-own-event
  ...
  (tu/with-terms [dog pet Fido]
    (let [[seen _] (recorder)
          wrote?   (atom false)]
      (v/watch kb (fn [e]
                    (swap! seen conj e)
                    (when (compare-and-set! wrote? false true)
                      (v/assert kb (list pet Fido) 'UniverseContext))))
      (v/assert kb (list dog Fido) 'UniverseContext)
      (is (= [[(list dog Fido)] [(list pet Fido)]]
             (mapv #(mapv :sentence (:believed-added %)) @seen))
          "two rounds, the second being what the listener itself wrote")
      (is (v/ask? kb (list pet Fido) 'UniverseContext)))))
...
(tu/deftest-kb a-listeners-own-writes-are-not-the-batchs-consequences
  ...
  (tu/with-terms [dog Fido sideEffect Yes]
...
(tu/deftest-kb a-listener-that-writes-on-every-event-terminates-at-the-bound
  ...
  (tu/with-terms [dog Fido]
    (let [rounds (atom 0)
      ...
      (is (v/ask? kb (list dog Fido) 'UniverseContext) "and the KB is usable after"))))
...
(tu/deftest-kb a-standing-query-does-not-render-what-it-did-not-match
  ...
  (tu/with-terms [dog cat Fido Tom]
    (let [calls (atom 0)
...
(tu/deftest-kb a-standing-query-fires-only-on-what-answers-it
  (tu/with-terms [dog cat Fido Tom]
    (let [[seen f] (recorder)]
      ...
      (v/assert kb (list dog Fido) 'UniverseContext)
      (is (= [(list dog Fido)] (added @seen)))
      (is (= [{'?x Fido}] (mapv :bindings (:believed-added (first @seen))))
...
(tu/deftest-kb a-standing-query-is-answered-by-a-subtype-and-a-sub-predicate
  ...
  ;; re-run of the goal.
  (tu/with-terms [animal_ dog_ Fido parentOf fatherOf Tom Bob]
    (let [[types tf] (recorder)
      ...
      (v/assert kb (list dog_ Fido) 'UniverseContext)
      (v/assert kb (list fatherOf Tom Bob) 'UniverseContext)
      (is (= [(list dog_ Fido)] (added @types)) "a subtype answers a supertype goal")
      (is (= [{'?x Fido}] (mapv :bindings (:believed-added (first @types)))))
...
(tu/deftest-kb a-standing-query-sees-what-its-context-sees-and-no-more
  (tu/with-terms [dog Fido Rex ChildContext ParentContext SiblingContext]
    (v/assert kb (list 'genlContext ChildContext ParentContext) 'UniverseContext)
    (let [[seen f] (recorder)]
      (v/watch kb (list dog '?x) ChildContext f)
      (v/assert kb (list dog Fido) ParentContext)
      (is (= [(list dog Fido)] (added @seen)) "up the genlContext cone, as any read is")
...
(tu/deftest-kb a-variable-context-watches-every-context-and-binds-the-one-that-answered
  (tu/with-terms [dog Fido Rex StoryContext OtherContext]
    (let [[seen f] (recorder)]
      (v/watch kb (list dog '?x) '?ctx f)
      (v/assert kb (list dog Fido) StoryContext)
      (v/assert kb (list dog Rex) OtherContext)
      (is (= [(list dog Fido) (list dog Rex)] (added @seen)))
      (is (= [{'?x Fido '?ctx StoryContext} {'?x Rex '?ctx OtherContext}]
...
(tu/deftest-kb a-standing-query-reports-what-left-belief-too
  (tu/with-terms [dog barks Fido]
    ...
    (let [[seen f] (recorder)]
      ...
      (v/assert kb (list 'not (list barks Fido)) 'UniverseContext {:strength :monotonic})
      (is (= [(list barks Fido)] (removed @seen)))
      (is (= [{'?x Fido}] (mapv :bindings (:believed-removed (first @seen)))))
      (is (= :defeated (:reason (first (:believed-removed (first @seen)))))))))
...
(tu/deftest-kb a-negated-goal-watches-the-negative-side-and-not-the-positive
  ...
  (tu/with-terms [dog Fido]
    (let [[pos pf] (recorder)
      ...
      (v/assert kb (list 'not (list dog Fido)) 'UniverseContext)
      (is (empty? @pos))
      (is (= [(list 'not (list dog Fido))] (added @neg)))
      (is (= [{'?x Fido}] (mapv :bindings (:believed-added (first @neg))))))))
