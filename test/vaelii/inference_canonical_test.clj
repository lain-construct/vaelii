...
(tu/deftest-kb every-node-the-search-builds-is-a-canonical-conjunction
  (tu/with-terms [edgeOf anc CanContext]
    (tu/with-terms [CnA CnB CnC]
      (chain-kb! kb edgeOf anc CanContext [CnA CnB CnC])
      (let [ns' (nodes-of kb [(list anc '?who '?whom)] CanContext)]
...
(tu/deftest-kb alpha-variant-queries-build-the-same-nodes
  ...
  (tu/with-terms [edgeOf anc AlphaContext]
    (tu/with-terms [AlA AlB AlC]
      (chain-kb! kb edgeOf anc AlphaContext [AlA AlB AlC])
      (let [shape (fn [goals] (set (map (fn [n] [(mapv :sentence (:literals n))
                                                 (mapv :depth (:literals n))])
                                        (nodes-of kb goals AlphaContext))))]
        ...
        (testing "and the answers come back under whichever names were used"
          (is (= #{'?who '?whom}
                 (set (mapcat keys (answers kb [(list anc '?who '?whom)] AlphaContext)))))
          (is (= #{'?x '?z}
                 (set (mapcat keys (answers kb [(list anc '?x '?z)] AlphaContext))))))))))
...
(tu/deftest-kb answer-terms-still-name-the-askers-variables-at-every-depth
  ...
  (tu/with-terms [edgeOf anc ComposeContext]
    (tu/with-terms [CoA CoB CoC]
      (chain-kb! kb edgeOf anc ComposeContext [CoA CoB CoC])
      (let [sess (inf/session kb [(list anc '?x '?z)] ComposeContext {:max-depth 3})
...
(tu/deftest-kb a-rewrite-that-grounds-a-query-variable-keeps-it
  ...
  (tu/with-terms [litOf fixedOf GroundContext]
    (tu/with-terms [GdOnly GdOther]
      (v/assert kb (list litOf GdOnly) GroundContext)
      (v/assert kb (list litOf GdOther) GroundContext)
      ;; the consequent pins its first argument, so proving (fixedOf ?a ?b) fixes ?a
      (v/assert-rule kb [(list litOf '?y)] (list fixedOf GdOnly '?y) GroundContext
                     {:direction :backward})
      (is (= #{{'?a GdOnly '?b GdOnly} {'?a GdOnly '?b GdOther}}
             (answers kb [(list fixedOf '?a '?b)] GroundContext 2))
...
(tu/deftest-kb a-rules-own-variables-never-reach-an-answer
  ...
  (tu/with-terms [edgeOf anc ScratchContext]
    (tu/with-terms [ScA ScB ScC ScD]
      (chain-kb! kb edgeOf anc ScratchContext [ScA ScB ScC ScD])
      (let [sols (answers kb [(list anc ScA '?z)] ScratchContext)]
        (is (seq sols))
...
(tu/deftest-kb a-query-spelled-in-the-engines-own-canonical-names-is-answered
  ...
  (tu/with-terms [edgeOf anc SwapContext]
    (tu/with-terms [SwA SwB SwC]
      (chain-kb! kb edgeOf anc SwapContext [SwA SwB SwC])
      ...
      (testing "and the crossed query answers the crossed relation"
        (let [straight (answers kb [(list anc '?var0 '?var1)] SwapContext)
              crossed  (answers kb [(list anc '?var1 '?var0)] SwapContext)]
          (is (seq straight))
...
(tu/deftest-kb one-rule-used-twice-on-a-path-does-not-meet-itself
  ...
  (tu/with-terms [edgeOf anc TwiceContext]
    (tu/with-terms [TwA TwB TwC TwD]
      (chain-kb! kb edgeOf anc TwiceContext [TwA TwB TwC TwD])
      (is (= #{TwB TwC TwD}
             (set (map #(get % '?z) (answers kb [(list anc TwA '?z)] TwiceContext 6))))
...
(tu/deftest-kb two-different-rules-spelled-alike-do-not-meet-each-other
  ...
  (tu/with-terms [linkOf midOf topOf TwoRuleContext]
    (tu/with-terms [TrA TrB]
      (v/assert kb (list linkOf TrA TrB) TwoRuleContext)
      (v/assert-rule kb [(list linkOf '?x '?y)] (list midOf '?x '?y) TwoRuleContext
                     {:direction :backward})
      (v/assert-rule kb [(list midOf '?x '?y)] (list topOf '?x '?y) TwoRuleContext
                     {:direction :backward})
      (is (= #{{'?a TrA '?b TrB}}
             (answers kb [(list topOf '?a '?b)] TwoRuleContext 3))))))
...
(tu/deftest-kb a-rule-whose-head-crosses-its-own-arguments-still-binds-the-right-way-round
  ...
  (tu/with-terms [fromTo toFrom InvContext]
    (tu/with-terms [IvA IvB]
      (v/assert kb (list fromTo IvA IvB) InvContext)
      (v/assert-rule kb [(list fromTo '?x '?y)] (list toFrom '?y '?x) InvContext
                     {:direction :backward})
      (is (= #{{'?p IvB '?q IvA}} (answers kb [(list toFrom '?p '?q)] InvContext 2))
          "the arguments came back the wrong way round")
      (testing "and with the head bound, both ways"
        (is (= #{{'?q IvA}} (answers kb [(list toFrom IvB '?q)] InvContext 2)))
        (is (empty? (answers kb [(list toFrom IvA '?q)] InvContext 2)))))))
...
(tu/deftest-kb a-guard-survives-being-lifted-through-two-renamings
  ...
  (tu/with-terms [wings flies odd airborne LiftContext]
    (tu/with-terms [LfBird LfPenguin]
      (doseq [i [LfBird LfPenguin]] (v/assert kb (list wings i) LiftContext))
      (v/assert kb (list odd LfPenguin) LiftContext)
      ...
      ;; sitting at the root
      (v/assert-rule kb [(list flies '?y)] (list airborne '?y) LiftContext
                     {:direction :backward})
      (testing "the exception still names the individual it was written about"
        (is (= #{LfBird} (set (map #(get % '?w)
                                   (answers kb [(list airborne '?w)] LiftContext 3)))))
        (is (empty? (answers kb [(list airborne LfPenguin)] LiftContext 3)))
        (is (= #{{}} (answers kb [(list airborne LfBird)] LiftContext 3)))))))
...
(tu/deftest-kb one-conjunction-asked-for-two-different-answers-is-two-nodes
  ...
  (tu/with-terms [pairOf bothOf SplitContext]
    (tu/with-terms [SpA SpB]
      (v/assert kb (list pairOf SpA SpB) SplitContext)
      (v/assert-rule kb [(list pairOf '?x '?y)] (list bothOf '?x '?y) SplitContext
                     {:direction :backward})
      (let [fwd (answers kb [(list bothOf '?m '?n)] SplitContext 2)
            rev (answers kb [(list bothOf '?n '?m)] SplitContext 2)]
        (is (= #{{'?m SpA '?n SpB}} fwd))
        (is (= #{{'?n SpA '?m SpB}} rev))
...
(tu/deftest-kb the-same-question-twice-builds-the-same-search
  ...
  (tu/with-terms [edgeOf anc DetContext]
    (tu/with-terms [DtA DtB DtC]
      (chain-kb! kb edgeOf anc DetContext [DtA DtB DtC])
      (let [lits (fn [n] (mapv :sentence (:literals n)))
            run  (fn []
                   (let [s (inf/session kb [(list anc '?x '?z)] DetContext {:max-depth 3})]
...
(tu/deftest-kb a-deferred-antecedent-is-still-computed-across-the-renaming
  (tu/with-terms [pairOf distinctPair DefCanContext]
    (tu/with-terms [DcA DcB]
      (v/assert kb (list pairOf DcA DcB) DefCanContext)
      (v/assert kb (list pairOf DcA DcA) DefCanContext)
      (v/assert-rule kb [(list pairOf '?x '?y) (list 'different '?x '?y)]
                     (list distinctPair '?x '?y) DefCanContext {:direction :backward})
      (is (= #{{'?p DcA '?q DcB}}
             (answers kb [(list distinctPair '?p '?q)] DefCanContext 2))))))

(tu/deftest-kb a-cyclic-rule-set-still-drains
  (tu/with-terms [pingOf pongOf CycCanContext]
    (tu/with-terms [CcInd]
      (v/assert-rule kb [(list pongOf '?x)] (list pingOf '?x) CycCanContext
                     {:direction :backward})
      (v/assert-rule kb [(list pingOf '?x)] (list pongOf '?x) CycCanContext
                     {:direction :backward})
      (let [sess (inf/session kb [(list pingOf CcInd)] CycCanContext {:max-depth 6})]
...
(tu/deftest-kb a-leaf-solver-answers-what-the-search-will-not-rewrite
  ...
  (tu/with-terms [edgeOf anc LeafContext]
    (tu/with-terms [LfA LfB LfC]
      (chain-kb! kb edgeOf anc LeafContext [LfA LfB LfC])
      (let [facts-only (set (inf/solutions kb [(list anc '?x '?z)] LeafContext {:max-depth 4}))
            registry   (set (inf/backchain kb (list anc '?x '?z) LeafContext
      ...
      (testing "and a leaf only a prover can answer is reached through it"
        ...
        (tu/with-terms [pairOf distinctOf]
          (v/assert kb (list pairOf LfA LfB) LeafContext)
          (v/assert kb (list pairOf LfA LfA) LeafContext)
          (v/assert-rule kb [(list pairOf '?x '?y) (list 'different '?x '?y)]
                         (list distinctOf '?x '?y) LeafContext {:direction :backward})
          (is (= #{{'?m LfA '?n LfB}}
                 (set (inf/backchain kb (list distinctOf '?m '?n) LeafContext
