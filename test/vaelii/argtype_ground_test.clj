...
(ns vaelii.argtype-ground-test
  ...
  The spy counts the `arg` probes `inferred-types` issues — exactly the work the
  ...
  never an `arg` probe and never counted."
...
(defn- counting-argisa-probes
  "Wrap `matches-visible`, counting only the `(arg …)` probes `inferred-types`
...
(defn- typed-individual-in-m-relations!
  ...
  arg-constrained at argument 1 to `animal` — so x fills M arg-constrained
  ...
  (let [dog (tu/tmp-type "dog") animal (tu/tmp-type "animal")]
    ...
    (dotimes [_ m]
      (let [rel (tu/tmp-pred "rel") other (tu/tmp-ind "Other")]
        (v/assert kb (list 'arg rel 1 animal) ctx)
...
(tu/deftest-kb stored-fact-ground-ask-does-not-scan-the-individuals-relations
  ;; THE gate.  With `(dog Muffet)` stored and Muffet also filling 30 arg-constrained
  ...
  ;; scan) this issued ~M arg probes before FactProver got a turn.
...
(tu/deftest-kb inferred-types-streams-rather-than-computing-the-whole-type-set
  ...
  ;; Eager (`into #{}`) would issue one arg probe per relation regardless.
  (tu/with-terms [Blob]
...
(tu/deftest-kb inferred-only-membership-still-holds
  ;; (food Bone1) is provable only through (arg eats 2 food) + (eats Muffet Bone1):
...
(tu/deftest-kb ground-miss-with-no-witness-returns-false
  ;; (cat Muffet) with no stored fact and no arg path: ArgTypeProver's `some` exhausts
