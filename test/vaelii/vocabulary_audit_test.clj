...
(ns vaelii.vocabulary-audit-test
  ...
  a term `CoreContext` declares that the roster says nothing about fails here, and so
  does a roster entry naming a term the grammar has retired.

  A CoreContext baseline is the whole population, which is why this is its own namespace
...
(tu/deftest-kb every-term-the-grammar-declares-is-classified
  ;; The durable half.  A functor added to CoreContext without anybody deciding whether
  ...
  (let [a (v/vocabulary-audit kb)]
    (is (empty? (:unclassified a))
        "a term CoreContext declares that vocabulary/roster says nothing about")
    (is (empty? (:retired a))
        "a roster entry naming a term CoreContext no longer declares")
