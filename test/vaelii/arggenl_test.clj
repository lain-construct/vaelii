...
(defn- type-relation
  ...
  (let [rel (tu/tmp-pred) root (tu/tmp-type) sub (tu/tmp-type) other (tu/tmp-type)]
    (v/assert kb (list 'genl root 'thing) 'UniverseContext)
    (v/assert kb (list 'genl sub root) 'UniverseContext)
    (v/assert kb (list 'genl other 'thing) 'UniverseContext)   ; a type, but not under root
    (v/assert kb (list 'argGenl rel 1 root) 'UniverseContext)
    [rel sub other root]))
...
(tu/deftest-kb argGenl-wants-a-subtype-where-argIsa-wants-an-instance
  ...
  (let [[rel sub] (type-relation kb)
    ...
    (v/assert kb (list 'genl root2 'thing) 'UniverseContext)
    (v/assert kb (list 'genl sub root2) 'UniverseContext)
    (v/assert kb (list 'argIsa instRel 1 root2) 'UniverseContext)
    ...
    (let [meta (tu/tmp-type)]
      (v/assert kb (list 'genl meta 'thing) 'UniverseContext)
      (v/assert kb (list meta sub) 'UniverseContext))
    ...
    (testing "an instance of the kind is what argIsa wanted"
      (let [x (tu/tmp-ind)]
        ...
        (is (v/assert kb (list instRel x (tu/tmp-ind)) 'UniverseContext))))))
...
(tu/deftest-kb a-derived-argGenl-violation-is-recorded-not-thrown
  ...
  (let [[rel _ other] (type-relation kb)
    ...
    (v/assert kb (list 'set/forwardRule
              ...
              'UniverseContext)
    (v/assert kb (list trigger other) 'UniverseContext)
...
(tu/deftest-kb both-constraints-on-one-position-narrow-it-rather-than-emptying-it
  ...
  (tu/with-terms [rel a_collection an_animal a_dog Rex]
    (v/assert kb (list 'genl a_collection 'thing) 'UniverseContext)
    (v/assert kb (list 'genl an_animal 'thing) 'UniverseContext)
    (v/assert kb (list 'genl a_dog an_animal) 'UniverseContext)
    (v/assert kb (list a_collection a_dog) 'UniverseContext)  ; a_dog *is* a collection

    (testing "both may be declared of one position"
      (is (v/assert kb (list 'argIsa rel 1 a_collection) 'UniverseContext))
      (is (v/assert kb (list 'argGenl rel 1 an_animal) 'UniverseContext)))

    (testing "and a term satisfying both is admitted — an instance of the one, a subtype of the other"
      (is (v/assert kb (list rel a_dog (tu/tmp-ind)) 'UniverseContext)))
...
(tu/deftest-kb the-constraint-must-agree-with-the-declared-relation-kind
  (let [instRel (tu/tmp-pred) typeRel (tu/tmp-pred)]
    (v/assert kb (list 'instanceRelationPredicate instRel) 'UniverseContext)
    ...
    (testing "an unclassified relation takes either"
      (let [rel (tu/tmp-pred)]
        ...
        (is (v/assert kb (list 'argGenl rel 2 'thing) 'UniverseContext))))))

(tu/deftest-kb arity-is-functional-so-a-second-value-is-refused
  ;; the declaration ships in CoreContext; this KB is empty, so it states it — that
  ;; the shipped vocabulary carries it is core-context-test's assertion
  (let [rel (tu/tmp-pred)]
    (v/assert kb '(functional arity) 'UniverseContext)
    (v/assert kb (list 'arity rel 2) 'UniverseContext)
  ...
  (let [rel (tu/tmp-pred) a (tu/tmp-ind) b (tu/tmp-ind)]
    ...
    (testing "open-world: an undeclared predicate takes any arity"
      (let [undeclared (tu/tmp-pred)]
        ...
        (is (v/assert kb (list undeclared a b) 'UniverseContext))))))
...
(tu/deftest-kb the-arity-declaration-can-come-from-either-spelling
  ...
  (let [byArity (tu/tmp-pred) byType (tu/tmp-pred) a (tu/tmp-ind)]
    (v/assert kb (list 'arity byArity 1) 'UniverseContext)
...
(tu/deftest-kb a-rebuild-clears-the-arity-cache-before-replaying-it
  ...
  (tu/with-cleared-kb [kb2 tu/fresh]
    (let [rel (tu/tmp-pred) a (tu/tmp-ind)]
      (v/assert kb2 (list 'arity rel 2) 'UniverseContext)
      ...
      (is (v/assert kb2 (list rel a) 'UniverseContext)
...
(tu/deftest-kb the-arity-a-reader-sees-is-the-one-that-binds-it
  ...
  (tu/with-terms [LeftContext RightContext BothContext]
    (let [rel (tu/tmp-pred) a (tu/tmp-ind) b (tu/tmp-ind)]
      (v/assert kb (list 'genlContext LeftContext 'UniverseContext) 'UniverseContext)
      (v/assert kb (list 'genlContext RightContext 'UniverseContext) 'UniverseContext)
      ...
      (v/assert kb (list 'arity rel 2) LeftContext)
      (v/assert kb (list 'arity rel 3) RightContext)
      (let [tax (:taxonomy kb)]
        (is (= 2 (tax/declared-arity tax rel LeftContext)) "sees only the binary one")
        (is (= 3 (tax/declared-arity tax rel RightContext)) "sees only the ternary one")
        (is (nil? (tax/declared-arity tax rel BothContext))
...
(tu/deftest-kb two-arities-for-one-predicate-constrain-nothing
  ...
  ;; gets here: `(functional arity)` merges the two values instead, and CoreContext
  ;; ships that declaration.  A KB without it, or an import that stored both, does.)
  (let [rel (tu/tmp-pred) a (tu/tmp-ind) b (tu/tmp-ind)]
    (v/assert kb (list 'arity rel 2) 'UniverseContext)
    (v/assert kb (list 'arity rel 3) 'UniverseContext)
    (is (nil? (tax/declared-arity (:taxonomy kb) rel))
