...
(deftest an-assumption-rule-is-distinct-from-its-bare-twin
  (tu/with-neutral-kb [kb tu/fresh]
    (tu/with-terms [candidate color]
      (let [bare (v/assert kb (list 'implies (list candidate '?c) (list color '?c 'red)) 'UniverseContext)
...
(deftest an-assumption-rule-does-not-forward-chain
  ...
  (tu/with-neutral-kb [kb tu/fresh]
    (tu/with-terms [candidate color Item]
      (v/assert kb (list 'implies (list candidate '?c) (list color '?c 'red)) 'UniverseContext)
      (v/assert kb (list 'set/assumptionRule (list 'implies (list candidate '?c) (list color '?c 'blue)))
                'UniverseContext)
      (v/assert kb (list candidate Item) 'UniverseContext)
      (is (seq (v/sentexes-matching kb (list color Item 'red) 'UniverseContext)) "bare rule fired")
      (is (empty? (v/sentexes-matching kb (list color Item 'blue) 'UniverseContext)) "assumptionRule did not"))))
...
(deftest an-inert-fact-is-stored-but-never-believed
  ...
  (tu/with-cleared-kb [kb tu/fresh]
    (tu/with-terms [p Tom BoxContext]
      (let [h (v/assert-inert kb (list p Tom) 'BoxContext)]
        (testing "stored and inspectable"
          (is (some? (sentex-of kb h)))
          (is (some #(= (:id %) h) (v/sentexes-in-context kb 'BoxContext))))
        (testing "but not a premise, so not believed"
          (is (not (v/in? kb h)))
          (is (empty? (v/sentexes-matching kb (list p Tom) 'BoxContext))))
        (testing "and retract! tears it down directly, though it was never a TMS datum"
          ...
          (is (empty? (v/sentexes-in-context kb 'BoxContext)))
...
(deftest an-inert-negation-forms-no-nogood
  ...
  (tu/with-cleared-kb [kb tu/fresh]
    (tu/with-terms [p Tom BoxContext]
      (v/assert kb (list p Tom) 'UniverseContext)                              ; believed
      (v/assert kb (list 'genlContext 'BoxContext 'UniverseContext) 'UniverseContext {:strength :monotonic})
      (v/assert-inert kb (list 'not (list p Tom)) 'BoxContext)                 ; inert opposite
      (testing "no contradiction, and the believed side stands"
        (is (zero? (count (v/contradictions kb))))
        (is (seq (v/sentexes-matching kb (list p Tom) 'UniverseContext)))))))
...
(defn- color-choices
  ...
  (let [candidate (tu/tmp-pred) color (tu/tmp-pred) item (tu/tmp-ind)]
    ...
    (v/assert kb (list 'set/assumptionRule (list 'implies (list candidate '?c) (list color '?c 'blue)))
              'UniverseContext)
    (v/assert kb (list 'functional color) 'UniverseContext)
    (v/assert kb (list candidate item) 'UniverseContext)
  ...
  (when asp?
    (tu/with-cleared-kb [kb tu/fresh]
      (let [{:keys [color item]} (color-choices kb)
            r (v/assert kb (list 'do/label 'UniverseContext (tu/tmp-ctx "Plan")) 'UniverseContext)]
        ...
        (testing "base belief is untouched — neither color is believed, no contradiction"
          (is (empty? (v/sentexes-matching kb (list color item 'red) 'UniverseContext)))
          (is (empty? (v/sentexes-matching kb (list color item 'blue) 'UniverseContext)))
          (is (zero? (count (v/contradictions kb)))))
    ...
    (tu/with-cleared-kb [kb tu/fresh]
      (tu/with-terms [candidate wants Item]
        (v/assert kb (list 'set/assumptionRule (list 'implies (list candidate '?c) (list wants '?c 'tea)))
                  'UniverseContext)
        (v/assert kb (list candidate Item) 'UniverseContext)
        (let [r (v/assert kb (list 'do/label 'UniverseContext (tu/tmp-ctx "Plan")) 'UniverseContext)]
          (is (= 1 (:count r)))
...
(deftest label-with-no-choices-does-nothing
  (tu/with-neutral-kb [kb tu/fresh]
    (tu/with-terms [p Tom]
      (v/assert kb (list p Tom) 'UniverseContext)
      (let [r (v/assert kb (list 'do/label 'UniverseContext (tu/tmp-ctx "Plan")) 'UniverseContext)]
        (is (zero? (:count r)))
  ...
  (tu/with-cleared-kb [kb tu/fresh]
    (tu/with-terms [candidate banned wants Item Other]
      (v/assert kb (list 'exceptWhen (list banned '?c)
                ...
                'UniverseContext)
      (v/assert kb (list candidate Item) 'UniverseContext)
      (v/assert kb (list banned Item) 'UniverseContext)
      ...
      (when asp?
        (testing "an unbanned candidate is still offered"
          (v/assert kb (list candidate Other) 'UniverseContext)
          (let [r (v/assert kb (list 'do/label 'UniverseContext (tu/tmp-ctx "Plan")) 'UniverseContext)]
            (is (= 1 (:count r)))
...
(deftest groundings-are-never-stored
  ...
  (when asp?
    (tu/with-cleared-kb [kb tu/fresh]
      (let [{:keys [color item]} (color-choices kb)
            r (v/assert kb (list 'do/label 'UniverseContext 'MenuPlanContext) 'UniverseContext)]
    ...
    (tu/with-cleared-kb [kb tu/fresh]
      (tu/with-terms [candidate color Item]
        ...
        (v/assert kb (list 'set/assumptionRule
                  ...
                  'UniverseContext)
        (let [fh   (v/assert kb (list 'functional color) 'UniverseContext)
              cand (v/assert kb (list candidate Item) 'UniverseContext)
              r1   (v/assert kb '(do/label UniverseContext ReplacePlanContext) 'UniverseContext)]
          (is (= 2 (:count r1)) "the clash splits the optima: two worlds")
          (testing "the base moves — the clash is retracted — and a re-run replaces"
            (v/retract! kb fh)
            (let [r2 (v/assert kb '(do/label UniverseContext ReplacePlanContext) 'UniverseContext)]
              (is (= 1 (:count r2)) "no clash, one world keeping both choices")
              (testing "the surplus stale context dropped out of the hierarchy"
                (is (contains? (set (v/contexts kb)) 'ReplacePlan1Context))
                (is (not (contains? (set (v/contexts kb)) 'ReplacePlan2Context))))
              (testing "and the surviving context holds exactly the new truth values"
                ...
                (is (= #{(list color Item 'red) (list color Item 'blue)}
                       (set (remove #(= 'labelingOf (first %))
                                    (map :sentence (v/sentexes-in-context kb 'ReplacePlan1Context)))))))))
          (testing "a run that grounds no choices clears everything: its result is 'no labelings'"
            (v/retract! kb cand)
            (let [r3 (v/assert kb '(do/label UniverseContext ReplacePlanContext) 'UniverseContext)]
              (is (= :no-choices (:reason r3)))
              (is (not (contains? (set (v/contexts kb)) 'ReplacePlan1Context))))))))))
  ...
  (when asp?
    (tu/with-cleared-kb [kb tu/fresh]
      (tu/with-terms [candidate color Item]
        ...
        (let [fh (v/assert kb (list 'functional color) 'UniverseContext)]
          (v/assert kb (list candidate Item) 'UniverseContext)
          (v/assert kb '(do/label UniverseContext ReclassPlanContext) 'UniverseContext)
          (let [c1 (v/assert kb '(do/classify ReclassPlanContext) 'UniverseContext)]
            (is (= 2 (count (:supportable c1))) "tied worlds: both colors supportable"))
          (v/retract! kb fh)
          (v/assert kb '(do/label UniverseContext ReclassPlanContext) 'UniverseContext)
          (let [c2 (v/assert kb '(do/classify ReclassPlanContext) 'UniverseContext)]
            ...
            (testing "the class context holds only the fresh classification"
              (is (= #{(list 'forced (list color Item 'red))
                     ...
                     (set (map :sentence (v/sentexes-in-context kb 'ReclassPlanClassContext))))))))))))
...
(deftest a-backendless-rerun-leaves-the-previous-run-standing
  ...
  (when asp?
    (tu/with-cleared-kb [kb tu/fresh]
      (color-choices kb)
      (let [r1 (v/assert kb '(do/label UniverseContext KeepPlanContext) 'UniverseContext)]
        (is (= 2 (:count r1)))
        (with-redefs [solver/available? (constantly false)]
          (let [r2 (v/assert kb '(do/label UniverseContext KeepPlanContext) 'UniverseContext)]
            (is (= :no-backend (:reason r2)))))
        (testing "nothing was cleared"
          (is (contains? (set (v/contexts kb)) 'KeepPlan1Context))
          (is (contains? (set (v/contexts kb)) 'KeepPlan2Context))
          ;; two truth values plus the labelingOf marker, all still standing
          (is (= 3 (count (v/sentexes-in-context kb 'KeepPlan1Context)))))))))
  ...
  (when asp?
    (tu/with-cleared-kb [kb tu/fresh]
      (tu/with-terms [p Tom]
        ...
        (v/assert kb '(genlContext SweepPlan1Context UniverseContext) 'UniverseContext
                  {:strength :monotonic})
        (v/assert kb (list p Tom) 'SweepPlan1Context {:strength :monotonic})
        (v/assert kb '(do/label UniverseContext SweepPlanContext) 'UniverseContext)
        (v/assert kb '(do/label UniverseContext SweepPlanContext) 'UniverseContext)
        (testing "the user's believed fact survived two runs of the sweep"
          (is (seq (v/sentexes-matching kb (list p Tom) 'SweepPlan1Context))))
        (testing "and classify aggregates only the marked labelings"
          (let [c (v/assert kb '(do/classify SweepPlanContext) 'UniverseContext)]
            (is (= 2 (:count c)))
  ...
  (when asp?
    (tu/with-cleared-kb [kb tu/fresh]
      (let [{:keys [color item]} (color-choices kb)
        ...
        (v/assert kb (list 'do/label 'UniverseContext into) 'UniverseContext)
        (let [c (v/assert kb (list 'do/classify into) 'UniverseContext)]
...
(deftest the-imperatives-check-their-arguments
  (tu/with-neutral-kb [kb tu/fresh]
    (doseq [form [(list 'do/label 'UniverseContext)                 ; one arg
                  (list 'do/label 'UniverseContext 'A 'B)           ; three
    ...
    (testing "and a do/ imperative is refused inside a rule"
      (is (thrown? clojure.lang.ExceptionInfo
                   (v/assert kb (list 'implies (list 'p '?x) (list 'do/label 'A 'B)) 'UniverseContext))))))
