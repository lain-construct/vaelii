...
(ns vaelii.spi-surface-test
  "The extension seams, pinned: the method set and arglists of every protocol a doc
  ...
  *behaves* — it runs the spec against each one, which is the stronger claim and the
  ...
  - **An ADDED method is breaking too**, which is the one people miss: an existing
    ...
    belongs on the seam, say so in the entry and update the in-tree implementers in the
    same change.
...
(def seams
  "The protocols a doc names as a seam, each with the doc that names it.  The pairing is
...
(defn regenerate-golden!
  ...
  regen-goldens` after a **deliberate** seam change, update every in-tree implementer in
  the same change, and commit the golden with it."
  []
  (spit golden-file
        (str ";; Frozen extension seams — the method sets an out-of-tree implementer\n"
             ";; compiles against. One entry per protocol a doc names as a seam.\n"
...
(deftest every-protocol-in-the-tree-is-classified
  ;; What makes the pinned set trustworthy. Without this, `seams` is a list somebody
  ...
  (let [declared (declared-protocols)
        pinned   (set (map (comp protocol-key first) seams))
        excused  (set (keys not-a-seam))
        stray    (set/difference declared pinned excused)]
    (is (empty? stray)
        (str "unclassified protocol(s): " (sort stray)
             ". Every protocol is either a seam somebody outside this repo implements —"
             " add it to `seams` with the doc that says so, and run `lein regen-goldens`"
             " — or one backend's internal shape, in which case add it to `not-a-seam`"
...
(deftest every-seam-names-a-doc-that-names-it-back
  ;; The entry's second half is a citation, so it is checked: a seam is pinned *because*
    ...
    (let [proto (str (:name (meta v)))
      ...
      (when (.isFile f)
        (is (str/includes? (slurp f) proto)
            (str proto ": " doc " no longer names it — either the seam moved doc, or"
                 " the page stopped inviting an implementation, in which case ask"
                 " whether it is still a seam."))))))
