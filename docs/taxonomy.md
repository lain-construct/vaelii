...
- `(functionalInArg P n)` — the same constraint with the *determined* position named
  ...
  `:partner-sweep-truncated` ([operations.md](operations.md)). The one shape settle-time
  discovery does **not** reach is a composite determinant with `n` *below* the arity —
  `(functionalInArg P 2)` on a ternary predicate — which needs a candidate rule keyed on
  several positions at once. The door checks it correctly whichever way it is written;
  it is cross-context *discovery* that stops there.
