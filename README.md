...
- JDK 21+, Leiningen 2.9+ — no external services
...
```clojure
(require '[vaelii.core :as v] '[vaelii.impl.starter :as starter])
...
;; a second open-kb over a space that already holds records warns and answers nothing
...
```sh
lein run -m vaelii.impl.web    # serves a starter-loaded KB on http://localhost:3000
```
...
dependency to a 1.1M-sentex KB, and the small vendored fixture to try it on first — is
[docs/kbs.md](docs/kbs.md).

## CLI & daemon

Drive a KB from the shell, or serve it over the network — both go through
`vaelii.core` alone. See [docs/operations.md](docs/operations.md).
